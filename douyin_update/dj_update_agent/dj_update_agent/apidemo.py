...
class DouYinApi:
    USER_AGENT = 'com.ss.android.ugc.aweme/1260 (Linux; U; Android 7.0; zh_CN; MT2-L05; Build/LMY47V; Cronet/58.0.2991.0)'
    ...
    URL_BASE = "http://139.196.84.224:9999/douyinapi/"

    PROXY = {
        #'http': 'http://111.76.169.180:15021',
        #'https': 'https://111.76.169.180:15021'
    }
    ...
    def get_user_info(self, user_id):
        ...
        douyin_url = 'https://api3-normal-c-lq.amemv.com/aweme/v1/user/profile/self/?'
        return self.__http_get(douyin_url, params)

    def get_sec_user_info(self, sec_user_id):
        """获取用户信息
        :param user_id: 用户ID
        :return:
        """
    ...
    def get_sec_user_post(self, sec_user_id, max_cursor, count):
        """获取用户作品
        :param user_id: 用户ID
        :param max_cursor: 用于分页，第1页是0，后1页是上1页请求的时候返回的max_cursor
        :param count: 返回视频的条数
    ...
    def user_search(self, keyword, offset, count):
        ...
        douyin_url = 'https://search.amemv.com/aweme/v1/discover/search/?'
        return self.__http_post(douyin_url, params)
    ...
    ## 根据关键词搜索话题
    def topic_search(self , hashtag_name ,cursor , count):
        ##
        #cursor=0&
        #keyword=%E8%80%81%E7%BD%97%E7%9B%B4%E6%92%AD&
        #count=20&
        #hot_search=0&
        #is_pull_refresh=0&source=challenge&search_source=search_sug&search_id=&query_correct_type=1&enter_from=homepage_hot
        params = {
            'cursor': str(cursor),
        ...
        douyin_url = 'https://search-lf.amemv.com/aweme/v1/challenge/search/?'
        return self.__http_get(douyin_url, params)

    ## 根据话题id 搜索相关视频  根据上一个接口返回的cid  就是 话题id
    def get_topic_videos(self, chid, cursor, count):
        """获取话题相关视频
        ...
        douyin_url = 'https://api3-normal-c-lf.amemv.com/aweme/v1/challenge/aweme/?'
        return self.__http_get(douyin_url, params)
    ...
    def __http_post(self, url, form_params=None):
        ...
        resp = requests.post(url, headers=self.__get_headers(sign), data=form_params, cookies=self.__cookie, proxies=self.PROXY)
        cookie = resp.cookies.get_dict()
        if len(cookie) != 0:
            self.__cookie.update(cookie)
            # print('cookie:' + str(self.__cookie))
...
douApi = DouYinApi("a3f25b1b5c44cef64cd79d5ced93bf3b")
try:
...
if __name__ == '__main__':
    ...
    result = douApi.get_user_info('81530232600')
    data_j = json.loads(result)
    print(data_j.get('user'))
    # result = douApi.get_user_following_list(96950096972, 0, 20);
    # result = douApi.get_sec_user_post('MS4wLjABAAAA_py8TGmFe6t8KDY04LU0JH9Yr9ml54dCjRFi0mc1lwI',0,20)
